...
def binary_focal_loss(gt, pr, gamma=2.0, alpha=0.25, **kwargs):
    ...
    # clip to prevent NaN's and Inf's
    pr = backend.clip(pr, backend.epsilon(), 1.0 - backend.epsilon())
    #mse = random_conv(gt, pr, backend)
    mse = backend.pow((pr - gt), 2)
...
def random_conv(gt, pr, backend):
    a = backend.random_normal_variable(shape=(12, 12, 1, 1), mean=0, scale=3)
    b = backend.random_normal_variable(shape=(8, 8, 1, 1), mean=4, scale=8)
    g_c = backend.conv2d(gt, a, strides=(2, 2), dilation_rate=(1, 1))
    g_c = backend.conv2d(g_c, b, strides=(2, 2), dilation_rate=(1, 1))
    p_c = backend.conv2d(pr, a, strides=(2, 2), dilation_rate=(1, 1))
    p_c = backend.conv2d(p_c, b, strides=(2, 2), dilation_rate=(1, 1))
    
    mse = backend.pow((p_c - g_c), 2)
    return mse
